...
public interface StuCafeGroupRepository extends JpaRepository<StuCafeGroup, Integer> {
    ...
    //Deena APIS
    @Query("SELECT scg.stuCafeGrpID,CONCAT(u.fName, ' ', u.lName) AS UserName,  c.cafeName AS CafeName FROM StuCafeGroup scg JOIN User u ON scg.userID = u.userID JOIN Cafe c ON scg.cafeID = c.cafeID WHERE scg.isAccepted = 0")
